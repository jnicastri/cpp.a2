...
namespace MyTicBase{
	...
	JuniorMyTic::JuniorMyTic() : ConcessionMyTic(0, 0){ }

	JuniorMyTic::JuniorMyTic(float theCredit, float theLimit) : ConcessionMyTic(theCredit, theLimit){ }
	...
	float JuniorMyTic::getDiscountRate(){

		return DISCOUNT_PERCENT;
	}

	void JuniorMyTic::printPurchases() const{
		...
		if (vSize != 0){
			for (int i = 0; i < vSize; i++){
				cout << count << ". "
					...
					<< " Concession Travel Pass purchased for $" << travelPasses[i]->getCost()
					<< endl;
				count++;
			}
		}
	...
	void JuniorMyTic::addPurchase(map<string, TravelPass*> &passes, time_t &journeyStartTime, time_t &journeyEndTime, Station &startStation, Station &endStation) throw (InsufficientFundsException){
