...
namespace MyTicBase{
	...
	FullMyTic::FullMyTic() : MyTic(0, 0){ }

	FullMyTic::FullMyTic(float theCredit, float theLimit) : MyTic(theCredit, theLimit){ }
	...
	void FullMyTic::printPurchases() const{
		...
		if (vSize != 0){
			for (int i = 0; i < vSize; i++){
				cout << count << ". "
					...
					<< " Adult Travel Pass purchased for $" << travelPasses[i]->getCost()
					<< endl;
				count++;
			}
		}
	...
	void FullMyTic::addPurchase(map<string, TravelPass*> &passes, time_t &journeyStartTime, time_t &journeyEndTime, Station &startStation, Station &endStation) throw (InsufficientFundsException){
